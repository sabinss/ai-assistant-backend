...
http.getPdfList = async (companyId, page, search, sortColumn, sortOrder, limit) => {
    const headers = {
        accept: "application/json",
    };
    const params = {
...
http.deleteFeedback = async (company_id, feedback_ids) => {
    const url = `/assistant/delete-feedbacks?company_id=${company_id}&feedback_ids=${feedback_ids.join("&feedback_ids=")}`;
...
http.deletePdf = async (company_id, file_names) => {
    const url = `/assistant/delete-pdfs?company_id=${company_id}&file_names=${file_names.join("&file_names=")}`;
    const headers = {
        accept: "application/json",
    };
    try {
