...
const createOrgToken = require('./seeders/organization-token-generate');
...
app.post('/webhook', async (req, res) => {
    ...
    if (entry && entry.length > 0 && entry[0].changes && entry[0].changes.length > 0) {
        ...
        if (change.value && change.value.messages && change.value.messages.length > 0) {
            ...
            if (answer) {
                const { whatsAppPhoneNumberId, whatsappToken } = organization.whatsappConfig;
...
app.post('/api/send-whatsapp', async (req, res) => {
    ...
    try {
        ...
        const organization = await Organization.findById(orgId).select('whatsappConfig');
        const { whatsappPhoneNumber, whatsAppPhoneNumberId, whatsappToken } = organization.whatsappConfig;
        console.log('whatsapp config', whatsappPhoneNumber, whatsAppPhoneNumberId, whatsappToken);
