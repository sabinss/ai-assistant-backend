...
exports.getFeedback = async (req, res) => {
    ...
    try {
        const feedback = await Feedback.findById(feedback_id).populate('conversation').populate('user');
...
exports.createFeedback = async (req, res) => {
    const { conversation, feedback, modified_answer, agentName = 'Onboarding Agent', feedbackMsg } = req.body;
    if (!conversation) {
...
exports.updateFeedback = async (req, res) => {
    try {
        ...
        const saveFeedback = await http.sendFeedback(feedback_id, req?.user?.organization, feedback);

        if (!updatedFeedback) {
            return res.status(404).json({ message: 'Feedback not found' });
        }

        res.status(200).json({ message: 'Feedback updated', updatedFeedback, saveFeedback });
    } catch (error) {
        console.log(error);
        res.status(500).json({ message: 'Internal Server Error', error });
    }
...
exports.createFeedbackSurvey = async (req, res) => {
    try {
        const { query, rating, feedback, user_email, organization_id, customer_id, user_id, agentName } = req.body;
...
exports.getFeedbackSurveys = async (req, res) => {
    try {
        ...
        const { organization_id, customer_id, user_id, updated_date, created_date } = req.query;
        // Build the filter object
        const filter = {};
        if (organization_id || req.organization) {
            filter.organization = organization_id ? organization_id : req.organization;
        }
        if (updated_date) {
