            ...
            for (const org of organizations) {
                ...
                for (const task of activeAgents) {
                    try {
                        ...
                        }/task-agent?task_name=${encodeURIComponent(task.name)}&org_id=${org._id}`;

                        const response = await axios.post(pythonServerUri);
                        console.log(`✅ [${org._id}] Task: ${task.name} responded with status ${response.status}`);
                    } catch (err) {
...
const shouldTriggerNow = (org, now, agentId) => {
    ...
    switch (frequency) {
        ...
        case 'Weekly':
            ...
            logger.info(`📅 [Agent ${agentId}] Weekly check: Today=${todayWeekDay}, Org=${orgWeekDay} → ${isWeekly}`);
        ...
        case 'Monthly':
            ...
            logger.info(`📆 [Agent ${agentId}] Monthly check: Today=${todayDate}, Org=${orgDay} → ${isMonthly}`);
            return isMonthly;
        case 'Quarterly':
            ...
            for (const agent of activeAgents) {
                if (shouldTriggerNow(agent, now, agent._id)) {
                    ...
                    try {
                        ...
                        logger.error(`❌ Error during cron job: ${error.stack || error.message}`);
