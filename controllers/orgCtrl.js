...
exports.create = async (req, res) => {
    ...
    try {
        ...
        const item = await User.findByIdAndUpdate(req.user._id, { $set: { organization: new_org._id } }, { new: true });
...
exports.editOrg = async (req, res) => {
    try {
        ...
        if (req.body?.singleUpdate) {
            ...
            const org = await Organization.findByIdAndUpdate(req?.user?.organization, payload, { new: true });
            return res.json(org);
        }
...
exports.getConnectedGmailsWithOrg = async (req, res) => {
    try {
        ...
        if (isVerifiedFromExternalCall) {
            ...
            const orgDetail = await Organization.findById(req.organization._id, 'orgGoogleCredential').lean();
...
exports.getCustomerDetail = async (req, res) => {
    ...
    try {
        ...
        const { _id, name, email, crm_cust_id, csm_cust_id, accounting_cust_id, help_desk_cust_id } = customer;
        res.status(200).json({
            data: {
                _id,
                name,
...
exports.updateOrgTaskAgents = async (req, res) => {
    try {
        ...
        const updatedTaskAgent = await TaskAgentModel.findByIdAndUpdate(taskAgentId, updateData, { new: true });
        if (!updatedTaskAgent) {
...
exports.fetchSourceFileList = async (req, res) => {
    try {
        ...
        const url = `${process.env.NEXT_PUBLIC_OPEN_API_FOR_CHAT}/assistant/get-pdfs-list?company_id=${
            params.company_id
        }&page=${params.page}&search=${params.search ?? null}&sortField=${params.sortField ?? null}&sortDirection=${
