...
exports.addConversation = async (req, res) => {
    try {
        const { question, chatSession } = req.body;
        const ans = await http.sendMessage(req?.user?.organization, question, chatSession);
        console.log(ans)
...
exports.getConversationByUserId = async (req, res) => {
    try {
        ...
        const conversation = await Conversation.find(searchCondition).sort({ createdAt: -1 });

        if (!conversation || conversation.length === 0) {
            return res.status(404).json({ error: 'Conversation not found for the provided user_id' });
        }
...
exports.updateLikeDislike = async (req, res) => {
    try {
        ...
        res.json({ message: 'Conversation updated successfully', updatedConversation });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
};
...
exports.totalConversations = async (req, res) => {
    try {
        const conversation = await Conversation.find({ organization: req.user.organization }).count();
        res.json(conversation);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.getPublicConversationByUserId = async (req, res) => {
    ...
    try {
        const conversation = await Conversation.find({ user_id: req.public_user_id, chatSession: chat_session }).sort({ created_date: -1 });
        res.json(conversation);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
exports.addPublicConversation = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            error: err.message + " SOMETWTHING WENT WROTG " + process.env.NEXT_PUBLIC_OPEN_API_FOR_CHAT + process.env.NEXT_PUBLIC_OPEN_API_FOR_CHAT_KEY,
