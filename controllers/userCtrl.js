...
exports.getUser = async (req, res) => {
  ...
  try {
    const user = await User.findById(user_id).select("-password").populate('role').populate('status');
...
exports.update = async (req, res) => {
  try {
    const user_id = req.params.user_id;
    const { first_name, last_name, email, role, newPassword, status, currentPassword } = req.body;
    let data;
    ...
    if (newPassword) {
      data = { first_name, last_name, email, role, status, password: bcrypt.hashSync(newPassword, 10) };
    }
    else {
...
exports.updateProfile = async (req, res) => {
  try {
    ...
    const { first_name, last_name, email, newPassword, status, currentPassword } = req.body;
    let data;
    let user = await User.findById(user_id);
    if (!user) {
      return res.status(404).json({ message: "User not found" });
    ...
    // add logic to only change new password if it is organization here.
    if (newPassword) {
      data = { first_name, last_name, email, status, password: bcrypt.hashSync(newPassword, 10) };
    }
    else {
...
exports.changeSession = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json({ message: "Session changed successfully", newSession });
  }
  catch (error) {
    console.error(error);
