...
app.post('/api/events', async (req, res) => {
  ...
  const { feature_id, email, device = 'web', organization } = req.body;
  const url =
    'https://grjr9d0i7k.execute-api.us-east-2.amazonaws.com/dev/api/customer_features';
  const payload = {
    email: email,
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: 'Event tracked successfully', data: response.data });
  } catch (err) {
    console.log('err', err);
    res.status(200).json({ message: 'Event tracking failed', error: err });
  }
});
